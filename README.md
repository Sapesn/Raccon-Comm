...
**v0.5.1 - 2026-02-24**
- ✅ 讨论详情页稳定性修复：移除客户端 `notFound()` 调用，改为兜底空状态页（避免客户端路由异常）
- ✅ 讨论列表排序修复：支持中文相对时间解析（刚刚 / X 分钟前 / X 小时前 / X 天前），`最新回复` 与 `最新发布` 排序结果正确
- ✅ 登录页可用性优化：移除无效的 `/forgot-password` 跳转，改为「即将上线」文案，避免 404
- ✅ 数据库安全加固：`packages/db` 不再使用弱默认连接串，强制要求 `DATABASE_URL`
- ✅ 环境变量示例更新：`.env.example` 中数据库连接改为占位符模板，降低误配置风险
