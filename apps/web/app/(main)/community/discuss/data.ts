 ...
 *
 * 包含社区论坛的帖子和回复数据
 * 支持经验分享、问题求助、行业讨论等多种交流场景
 */

/**
 * 社区讨论帖列表 Mock 数据
 * 包含 7 个不同类型的讨论帖，涵盖最佳实践、行业应用、问题求助等场景
 *
 * 字段说明：
 * - id: 帖子唯一标识
 * - title: 帖子标题
 * - author: 发帖者昵称
 * - avatar: 发帖者头像（单字母简写）
 * - avatarGrad: 头像渐变色类名，用于 Tailwind 渐变背景
 * - category: 帖子分类（经验分享、行业讨论、问题求助、使用心得、技术交流）
 * - content: 帖子正文内容，支持 Markdown 格式
 * - replies: 回复数量
 * - views: 浏览次数
 * - likes: 点赞数
 * - createdAt: 发帖时间（相对时间格式）
 * - lastReply: 最后回复时间（相对时间格式），用于热度排序
 * - tags: 帖子标签列表，用于内容聚合和检索
 */
export const POSTS = [
  {
    id: '1',
 ...
 * 字段说明：
 * - id: 回复唯一标识
 * - postId: 所属帖子 ID，关联到 POSTS 中的帖子
 * - author: 回复者昵称
 * - avatar: 回复者头像（单字母简写）
 * - avatarGrad: 头像渐变色类名
 * - content: 回复内容文本
 * - likes: 回复获得的点赞数
 * - createdAt: 回复时间（相对时间格式）
 */
